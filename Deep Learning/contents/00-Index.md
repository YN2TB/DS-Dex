...
**No errata filed. Nine discrepancies investigated and declined.** Any discrepancy between a printed number and a recomputation is logged here **only after** ruling out extraction damage, my own arithmetic, and an alternative convention — filing a false erratum against a correct source is the worse failure.
...
| D7 | §12.9.2, and §12.8.1 | "Choosing $\rho = 0.9$ amounts to a **half-life time of 10** for each parameter update" | For weights $(1-\rho)\rho^t$ the **true half-life** is $\ln 0.5/\ln 0.9 = \mathbf{6.5788}$; **10** is the effective sample size $1/(1-\rho)$ and **9** is the mean lag $\rho/(1-\rho)$. Cumulative weight after 10 lags is $1-0.9^{10} = \mathbf{65.1\%}$, not 50%. | **Declined as an erratum — recorded as a reading hazard.** The computed quantity is correct and "half-life" is used loosely for a time constant in several literatures. But the three numbers (6.58 / 9 / 10) are genuinely different, and which one a source means changes how you read every leaky average in the chapter. Flagged in ch. 04 §20. |
| D8 | §8.1.2, on AlexNet's fully connected layers | "two huge fully connected layers with 4096 outputs. These layers require **nearly 1GB** model parameters" | The two layers are $9216\times4096$ and $4096\times4096$ = **54,534,144 parameters = 208.0 MB** in fp32 — **4.9× less than 1 GB**. | **Declined as an erratum — recorded as an imprecision.** Ruled out own extraction (clean prose), own arithmetic (the full total agrees with the widely quoted ~60M for AlexNet) and an abridged specification. **The alternative convention resolves it: weights + gradients + Adam's two moments is $4\times208.0 = 832.1$ MB, 'nearly 1GB'.** True of the *training footprint*, false of the *parameters* — which is ch. 04 §5's finding arriving from the other direction. Flagged in ch. 05 §9. |
| D9 | §9.2.5, on n-gram frequency distributions | "sequences of words also appear to be following Zipf's law, albeit with a **smaller exponent** ... depending on the sequence length" | Fitted from D2L's own printed top-10 frequencies: **unigram $\alpha=0.7184$, bigram $0.5703$, trigram $0.6447$**. Smaller-than-unigram holds for both; **the implied monotone decrease does NOT — the trigram exponent exceeds the bigram's**, on both fitting windows (ranks 1-10 and 2-10). | **Declined — recorded as a measurement, not an error.** Ruled out own arithmetic (two windows, identical ordering) and own extraction (clean printed lists). **The wording “depending on the sequence length” does not assert monotonicity, and ten ranks of the HEAD — which D2L itself calls “exceptions” — cannot settle it.** Full working in ch. 07 §4. |
ho = 0.9$ amounts to a **half-life time of 10** for each parameter update" | For weights $(1-
ho)
ho^t$ the **true half-life** is $\ln 0.5/\ln 0.9 = \mathbf{6.5788}$; **10** is the effective sample size $1/(1-
ho)$ and **9** is the mean lag $
ho/(1-
ho)$. Cumulative weight after 10 lags is $1-0.9^{10} = \mathbf{65.1\%}$, not 50%. | **Declined as an erratum — recorded as a reading hazard.** The computed quantity is correct and "half-life" is used loosely for a time constant in several literatures. But the three numbers (6.58 / 9 / 10) are genuinely different, and which one a source means changes how you read every leaky average in the chapter. Flagged in ch. 04 §20. |
