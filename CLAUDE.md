...
- **⚠️ THE VAULT IS NO LONGER COMPLETE — the user added documents for TWO NEW SUBJECTS.** Verified by listing `D:\NEU` directly: **24 → 26 folders.**
  - **Deep Learning** — `Dive into Deep Learning` (Zhang/Lipton/Li/Smola, PyTorch ed., 1185 pp.). **🚧 IN PROGRESS: `CLAUDE.md` + `00-Index` + ch. 01–04 written. Next: ch. 05 (Convolutional Neural Network — D2L ch. 7 entire + ch. 8 entire: LeNet, AlexNet, VGG, NiN, GoogLeNet, batch norm, ResNet, DenseNet).**
...
| **Deep Learning** | 🚧 **In progress — `00-Index` + ch. 01–04 of 08** (scope given by user's `note/Index.md`; 7 discrepancies declined) |
